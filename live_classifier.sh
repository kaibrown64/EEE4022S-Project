...
while :; do
	...
	if test -f "${ARFFS}LIVE.arff"; then
		echo "deleting ${ARFFS}LIVE.arff"
		sudo trash-put "${ARFFS}LIVE.arff"
	fi
	...
	sudo tcpdump -i enp0s3 -w "${PCAPS}LIVE.pcap" -c 100
	...
	sudo java -cp "${WEKADIR}weka.jar" weka.filters.unsupervised.attribute.Remove -R 1,2,3,4 -i "${ARFFS}netmate.arff" -o "${ARFFS}LIVE.arff"
	# Label the traffic with a ? class
	sudo java -cp "${WEKADIR}weka.jar" weka.filters.unsupervised.attribute.Add -T NOM -N resolution -L 144p,240p,360p,480p,720p -C last -W 1.0 -i "${ARFFS}LIVE.arff" -o "${ARFFS}LIVE1.arff"
	# Resample the recorded packets
	# sudo java -cp "${WEKADIR}weka.jar" weka.filters.supervised.instance.Resample -B 0 -S 1 -Z 100 -c last -i "${ARFFS}LIVE1.arff" -o "${ARFFS}LIVE2.arff"
	# Classify the .arff file and save results to .txt
	sudo java -cp "${WEKADIR}weka.jar" weka.classifiers.trees.RandomForest -T "${ARFFS}LIVE1.arff" -l "${APPDIR}/Models/100-Forest.model" -p 0 | tee -a "${APPDIR}classifications.txt"
